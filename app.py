...
# ==================== КОНСТАНТЫ ====================
MAX_SPLINE_POINTS = 5000  # Максимум точек для сплайна
SPLINE_FLATTENING = 0.01  # Точность аппроксимации сплайна (мм)
MAX_LENGTH = 1_000_000  # 1 км - аномально большая длина
MIN_LENGTH = 1e-6  # Минимальная значимая длина
BULGE_EPSILON = 0.0001  # Порог считания булжа за ноль
COORD_EPSILON = 1e-10  # Эпсилон для координат
...
class ErrorCollector:
    ...
    def get_summary_with_percent(self, total_objects):
...
def validate_length_result(length, entity_type, entity_num, collector):
    ...
    # ✅ FIX: Проверка на отрицательное значение
    if length < 0:
    ...
    # ✅ FIX: Проверка на аномально большое значение (> 1 000 000 мм = 1 км)
...
def calc_ellipse_length(entity):
    ...
    a = math.sqrt(
        float(major_axis.x)**2 + 
        float(major_axis.y)**2 + 
        float(major_axis.z)**2
    )
    b = a * ratio
    ...
    if angle_span > 2 * math.pi:
        angle_span = 2 * math.pi
    
    # ✅ FIX: Проверка на полный эллипс (с допуском)
    ...
    N = min(1000, int(angle_span * 100))  # Адаптивное количество шагов
    length = 0.0
...
def calc_lwpolyline_length(entity):
    ...
    # ✅ FIX: Правильное определение сегментов
    # Для открытой полилинии: от 0 до n-1
    # Для закрытой: от 0 до n (замыкаем последний сегмент)
    num_segments = len(points) if entity.closed else len(points) - 1
    
    for i in range(num_segments):
        curr_idx = i
        next_idx = (i + 1) % len(points)
        
        x1 = float(points[curr_idx][0])
        y1 = float(points[curr_idx][1])
        x2 = float(points[next_idx][0])
        y2 = float(points[next_idx][1])
        
        bulge = float(points[curr_idx][2]) if len(points[curr_idx]) >= 3 else 0.0
        
        # ✅ FIX: Защита от некорректного bulge
        ...
        # ✅ FIX: Если булж близко к нулю, это прямая линия
        if abs(bulge) < BULGE_EPSILON:
            ...
            if abs(sin_half) < COORD_EPSILON:
                ...
                # ✅ FIX: Защита от аномальных значений
...
def calc_polyline_length(entity):
    ...
    # ✅ FIX: Правильная проверка для замкнутой полилинии
...
def calc_spline_length(entity):
    """SPLINE: сплайн."""
    try:
        # ✅ FIX: Ограничиваем количество точек для защиты от утечки памяти
        points = list(entity.flattening(SPLINE_FLATTENING))
        
        if len(points) > MAX_SPLINE_POINTS:
            logger.warning(
                f"Сплайн имеет {len(points)} точек, ограничиваем до {MAX_SPLINE_POINTS}"
            )
            # Берём равномерно распределённые точки
            step = len(points) // MAX_SPLINE_POINTS
            points = points[::step]
...
def calc_point(entity):
    """POINT: точка."""
    # Точки имеют нулевую длину реза, но показываем их как очень малую длину
    return 0.0


def calc_mline_length(entity):
    """MLINE: мультилиния."""
    # Мультилинии - это сложные объекты, пропускаем
    return 0.0


def calc_insert_length(entity):
    """INSERT: блок."""
    # Блоки не имеют прямой длины реза
    return 0.0


def calc_text_length(entity):
    """TEXT: текст."""
    # Текст не имеет длины реза
    return 0.0
...
def get_entity_center_with_offset(entity, offset_distance, collector=None, entity_num=0):
    ...
    Returns:
        tuple: (x, y) координаты центра со смещением или (0, 0) при ошибке
    """
    ...
    try:
        if entity_type == 'LINE':
            ...
            center_x = (float(s.x) + float(e.x)) / 2
            center_y = (float(s.y) + float(e.y)) / 2
            
            dx = float(e.x) - float(s.x)
            dy = float(e.y) - float(s.y)
            ...
            radius = float(entity.dxf.radius)
            return (float(center.x) + radius + offset_distance, float(center.y))
            ...
            return (
                float(center.x) + (radius + offset_distance) * math.cos(mid_angle),
                float(center.y) + (radius + offset_distance) * math.sin(mid_angle)
            )
            ...
            return (float(center.x) + a + offset_distance, float(center.y))
        
        elif entity_type == 'POINT':
            loc = entity.dxf.location
            return (
                float(loc.x) + offset_distance,
                float(loc.y) + offset_distance
            )
            ...
            if len(points) >= 2:
                ...
                if mid_idx + 1 < len(points):
                    dx = float(points[mid_idx + 1][0]) - float(points[mid_idx][0])
                    dy = float(points[mid_idx + 1][1]) - float(points[mid_idx][1])
                    seg_length = math.hypot(dx, dy)
            ...
            return (float(pos.x) + offset_distance, float(pos.y) + offset_distance)
...
def draw_entity_manually(ax, entity, collector=None):
    ...
    try:
        if entity_type == 'LINE':
            ...
            x = [
                float(center.x) + radius * math.cos(math.radians(t))
                for t in theta
            ]
            y = [
                float(center.y) + radius * math.sin(math.radians(t))
                for t in theta
            ]
            ...
            a = math.sqrt(
                float(major_axis.x)**2 + float(major_axis.y)**2
            )
            b = a * ratio
            angle = math.atan2(float(major_axis.y), float(major_axis.x))
            
            t = [i * 2 * math.pi / 100 for i in range(101)]
            x = [
                float(center.x) + a * math.cos(ti) * math.cos(angle)
                                - b * math.sin(ti) * math.sin(angle)
                for ti in t
            ]
            y = [
                float(center.y) + a * math.cos(ti) * math.sin(angle)
...
def visualize_dxf_with_numbers(doc, objects_data, show_markers=True,
    ...
    Args:
        doc: ezdxf document
        objects_data: список объектов с их параметрами
        show_markers: показывать ли маркеры
        font_size_multiplier: множитель размера шрифта
        collector: ErrorCollector для логирования
    ...
    try:
        ...
        if show_markers and objects_data:
            # ✅ FIX: Фильтруем валидные координаты
            valid_objects = [
                obj for obj in objects_data
                if not (obj['center'][0] == 0 and obj['center'][1] == 0)
            ]
            ...
            if valid_objects:
                ...
                # ✅ FIX: Используем id() вместо rounded координат
                # для избежания потери точности
                entity_list = list(msp)
                entity_by_id = {}
                
                for ent in entity_list:
                    if ent.dxftype() in calculators:
                        entity_by_id[id(ent)] = ent
                
                # Рисуем маркеры
                for obj_idx, obj in enumerate(objects_data):
                    cx, cy = obj['center']
                    
                    # Находим соответствующий entity по индексу
                    calc_count = 0
                    entity = None
                    
                    for ent in entity_list:
                        if ent.dxftype() in calculators:
                            if calc_count == obj_idx:
                                entity = ent
                                break
                            calc_count += 1
...
if uploaded_file is not None:
    with st.spinner('⏳ Обработка чертежа...'):
        ...
        try:
            ...
            # ✅ FIX: Получаем все объекты один раз
            ...
            # ✅ FIX: Правильная нумерация всех объектов
            real_object_num = 0
            ...
            for entity in all_entities:
                ...
                # ✅ FIX: Безопасный расчёт с записью ошибок
                length = calc_entity_safe(
                    entity_type, entity, real_object_num, calculators, collector
                )
                
                # Пропускаем объекты с нулевой длиной
                if length < MIN_LENGTH:
                    if entity_type not in ('MLINE', 'INSERT', 'TEXT', 'POINT'):
                        collector.add_skipped(
                ...
                object_display_num = len(objects_data) + 1
                
                objects_data.append({
                    'num': object_display_num,  # Порядковый номер в спецификации
                    'real_num': real_object_num,  # Реальный номер в файле
                    'type': entity_type,
                    'length': length,
            ...
            if not objects_data:
                ...
                total_problem_objects = (
                    len(collector.errors) + len(collector.warnings)
                )
    ...
    ✅ **Исправлена нумерация объектов** — теперь совпадает с реальным порядком в файле  
    ✅ **Защита от утечек памяти** — ограничение количества точек сплайна  
    ✅ **Исправлена обработка булжа** — корректный расчёт дуг в полилиниях  
    ✅ **Улучшена валидация** — проверка корректности координат  
    ✅ **Оптимизирована визуализация** — использование id() вместо округления  
    ✅ **Расширенное логирование** — процент проблем от общего количества  
    """)

st.markdown("---")
st.markdown("""
<div style='text-align: center; color: gray; font-size: 12px;'>
    ✂️ CAD Analyzer Pro v14.0 | Исправленная обработка | Поддержка DXF | 
